def simple_counting_sort(array)
  ...
  counts.each_with_index do |elem, index|
    counts[index] += counts[index-1] unless index.zero?
  end
  count_sort = Array.new(counts.last, 0)
  array.each do |elem|
    idx = counts[elem]
    count_sort[idx] = elem
    counts[elem] -= 1
...
def get_counts(array)
  array.each_with_object(Array.new(array.max+1,0)) { |elem, counts| counts[elem] += 1 }
end
